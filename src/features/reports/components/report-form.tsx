  ...
  return (
    <Field>
      ...
      <Select value={value} onValueChange={(nextValue) => onValueChange(nextValue ?? "")}>
        <SelectTrigger id={id} className="h-10 w-full bg-background/70"><SelectValue placeholder={placeholder} /></SelectTrigger>
...
export function ReportForm() {
  ...
  function resetCategory(nextCategory: string) { setCategory(nextCategory); setReportType(""); setLocation(""); setFacility(""); setWorkspace(""); setService(""); setProgram(""); setAttachments([]); setSavedState("idle") }
  ...
  const canSubmit = Boolean(category) && (category !== "kehilangan-temuan" || Boolean(reportType)) && (category !== "fasilitas" || Boolean(facility && (location || workspace))) && (category !== "layanan" || Boolean(service && program))

  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1.45fr)_minmax(280px,0.55fr)]">
      <Card className="rounded-2xl border-border bg-sidebar p-1.5 text-sidebar-foreground shadow-xs"><div className="rounded-xl border border-border/60 bg-card text-card-foreground shadow-2xs"><CardHeader className="space-y-2 border-b border-border/60 p-5 md:p-6"><div className="flex items-center gap-2 text-xs font-medium text-muted-foreground"><FileText className="size-4 text-primary" aria-hidden="true" />Formulir laporan</div><CardTitle className="text-xl tracking-tight md:text-2xl">Sampaikan laporan dengan detail</CardTitle></CardHeader><CardContent className="p-5 md:p-6">
        <form className="space-y-7" onSubmit={handleSubmit}>
          ...
          {category ? <div className="space-y-5 border-t border-border/60 pt-6"><div className="flex items-center gap-2"><span className="flex size-7 items-center justify-center rounded-lg bg-primary/10 text-sm font-semibold text-primary">1</span><h2 className="text-sm font-semibold">Informasi laporan</h2></div><FieldGroup>
            ...
            <Field><FieldLabel htmlFor="title">Judul laporan</FieldLabel><Input id="title" name="title" className="h-10 bg-background/70" placeholder="Contoh: Dompet tertinggal di Ruang 3.4" required /></Field>
            {category === "kehilangan-temuan" ? <><Field><FieldLabel htmlFor="item-name">Nama barang</FieldLabel><Input id="item-name" name="item-name" className="h-10 bg-background/70" placeholder="Contoh: Dompet kulit warna hitam" required /></Field><Field><FieldLabel htmlFor="item-details">Ciri-ciri barang</FieldLabel><Textarea id="item-details" name="item-details" placeholder="Tuliskan ciri khas, isi, atau tanda pengenal barang." required /></Field></> : null}
            ...
            {category === "lainnya" ? <Field><FieldLabel htmlFor="other-category">Kategori umum</FieldLabel><Input id="other-category" name="other-category" className="h-10 bg-background/70" placeholder="Contoh: Usulan kegiatan atau informasi umum" required /></Field> : null}
            <div className="grid gap-5 sm:grid-cols-2"><Field><FieldLabel htmlFor="incident-date">Waktu kejadian</FieldLabel><Input id="incident-date" name="incident-date" type="datetime-local" className="h-10 bg-background/70" required /></Field>{category !== "fasilitas" ? <Field><FieldLabel htmlFor="location">Lokasi kejadian</FieldLabel><Input id="location" name="location" value={location} onChange={(event) => setLocation(event.target.value)} className="h-10 bg-background/70" placeholder="Contoh: Gedung JTI, Ruang 3.4" required /></Field> : <div className="flex items-end text-sm text-muted-foreground"><div className="flex items-start gap-2 rounded-lg bg-muted/60 p-3"><MapPin className="mt-0.5 size-4 shrink-0" aria-hidden="true" /><span>Lokasi fasilitas dipilih dari data ruangan JTI di atas.</span></div></div>}</div>
