  ...
  return (
    <div className="flex min-h-screen w-full gap-3 overflow-hidden bg-sidebar p-3 text-sidebar-foreground">
      <aside className="flex w-64 shrink-0 flex-col bg-sidebar px-3 py-6 max-md:w-16 max-md:px-2">
        <div className="flex items-center gap-3 px-1 py-2"><div className="flex size-10 items-center justify-center rounded-xl bg-sidebar-primary text-sidebar-primary-foreground"><Activity className="size-5" /></div><div className="max-md:hidden"><p className="font-semibold tracking-tight">LaporJTI</p><p className="text-xs text-muted-foreground">Workspace</p></div></div>
        <nav className="mt-10 space-y-2" aria-label="Main navigation"><p className="mb-3 px-2 text-xs font-medium uppercase tracking-wider text-muted-foreground max-md:hidden">Menu</p>{navigation.map(({ label, icon: Icon, active }) => <Button key={label} variant={active ? "secondary" : "ghost"} className="h-10 w-full justify-start gap-3 px-2 text-sm max-md:justify-center max-md:px-0"><Icon className="size-4" /><span className="max-md:hidden">{label}</span></Button>)}</nav>
        <div className="mt-auto space-y-2"><Button variant="ghost" className="h-10 w-full justify-start gap-3 px-2 max-md:justify-center max-md:px-0"><CircleHelp className="size-4" /><span className="max-md:hidden">Help center</span></Button><div className="mt-5 flex items-center gap-3 border-t border-sidebar-border px-1 pt-5"><Avatar className="size-9"><AvatarFallback>AS</AvatarFallback></Avatar><div className="min-w-0 max-md:hidden"><p className="truncate text-sm font-medium">Ayu Santoso</p><p className="truncate text-xs text-muted-foreground">Administrator</p></div></div></div>
      </aside>
